...
df2 = df[c(1:13, 15:18, 20:23, 25:31, 37)]
...
df_new2 = df3 %>%
  ...
  #filter(Wilks < 500) %>%
  filter(Equipment == "Raw") %>%
  #filter(BodyweightKg > 83.0) %>%
  #filter(BodyweightKg < 130) %>%
  filter(Sex == "M") %>%
  filter(Year > 2015) %>%
  filter(Event == "SBD")%>%
  filter(Division == "O") %>%
  filter(WeightClassKg == "83")
  
df_new = data.frame(df_new2$Sex, df_new2$Equipment, df_new2$Age, df_new2$BodyweightKg, df_new2$Squat1Kg,df_new2$Squat2Kg,df_new2$Squat3Kg,df_new2$Bench1Kg, df_new2$Bench2Kg, df_new2$Bench3Kg, df_new2$Deadlift1Kg,df_new2$Deadlift2Kg,df_new2$Deadlift3Kg, df_new2$TotalKg)

mod = lm(df_new2$TotalKg ~  -1 +df_new2$Squat1Kg + df_new2$Bench1Kg + df_new2$Deadlift1Kg + df_new2$Age + df_new2$BodyweightKg, df_new)
summary(mod)
